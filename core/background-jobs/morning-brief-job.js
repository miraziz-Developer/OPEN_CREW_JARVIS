...
function composeBrief({ now = new Date(), missions = [], usage = {}, yesterday = '', events = [], unread = [] }) {
  ...
  if (events.length) lines.push('Today: ' + events.slice(0, 5).map(e => `${String(e.start || '').slice(11, 16) || 'all day'} ${e.title}`).join('; ') + '.');
  if (unread.length) lines.push(`Unread mail: ${unread.length}${unread.length >= 10 ? '+' : ''} — ${unread.slice(0, 3).map(m => String(m.subject || '').replace(/\s+/g, ' ').slice(0, 60)).join(' | ')}.`);
...
function createMorningBriefJob({ projectDir, localDateStr, hour = 8, getMissions, getUsage, getYesterday, getEvents, getUnread, sendTelegram, announce, now = () => new Date() }) {
  ...
  async function run() {
    ...
    let missions = [], usage = {}, yesterday = '', events = [], unread = [];
    try { events = (await getEvents?.()) || []; } catch (_) {}
    try { unread = (await getUnread?.()) || []; } catch (_) {}
    try { missions = getMissions(); } catch (_) {}
    try { usage = getUsage(); } catch (_) {}
    try { yesterday = await getYesterday(); } catch (_) {}
    const text = composeBrief({ now: now(), missions, usage, yesterday, events, unread });
